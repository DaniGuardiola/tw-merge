...
test("basic arbitrary variants", () => {
  expect(twMerge("[&>*]:underline [&>*]:line-through")).toBe(
    "[&>*]:line-through"
  );
  expect(
    twMerge("[&>*]:underline [&>*]:line-through [&_div]:line-through")
  ).toBe("[&>*]:line-through [&_div]:line-through");
  expect(
    twMerge("supports-[display:grid]:flex supports-[display:grid]:grid")
  ).toBe("supports-[display:grid]:grid");
});

test("arbitrary variants with modifiers", () => {
  expect(
    twMerge("dark:lg:hover:[&>*]:underline dark:lg:hover:[&>*]:line-through")
  ).toBe("dark:lg:hover:[&>*]:line-through");
  expect(
    twMerge("dark:lg:hover:[&>*]:underline dark:hover:lg:[&>*]:line-through")
...
test("arbitrary variants with arbitrary properties", () => {
  expect(twMerge("[&>*]:[color:red] [&>*]:[color:blue]")).toBe(
    "[&>*]:[color:blue]"
  );
  expect(
    twMerge(
