...
import { GenerateTailwindRuleSetOptions, ResolvedRules, TailwindRules } from './types'
import { GenerationState, EMPTY_GENERATION_STATE } from './generation-state'

function resolveCategory(
    category: keyof Omit<TailwindRules, 'mode'>,
    config: boolean | (Record<string, boolean> & { mode?: 'whitelist' | 'blacklist' }),
...
function resolveRules({ mode = 'blacklist', ...categories }: TailwindRules): ResolvedRules {
    ...
    return {
        ...
        ...Object.fromEntries(
            Object.entries(UTILITIES_BY_CATEGORY).map(([_category, utilities]) => {
                ...
                return [category, resolveCategory(category, categories[category] as any, mode)]
            }),
        ),
    }
}
...
function updateImports(state: GenerationState) {
    const importConditions: Record<GenerationState['imports'][number], unknown[]> = {
        uniqueRule: [
