...
export const DEFAULT_TAILWIND_CONFIG: Config = [
    ...
    ...uniqueRules([BG_ATTACHMENT, BG_AND_OBJECT_POSITION, BG_REPEAT, BG_SIZE], { prefix: 'bg' }),
...
export const DEFAULT_TAILWIND_CONFIG_LITE: Config = [
    ...
    // these rules are ordered as in the Tailwind docs (more or less)
    ...simpleRules(
        'aspect|start|end|top|right|bottom|left|z|basis|flex|order|col-start|col-end|col|row-start|row-end|row|justify-self|self|place-self|w|min-w|max-w|h|min-h|max-h|align',
    ),
    ...uniqueRules([DISPLAY, POSITION, VISIBILITY]),
    uniqueRule(OBJECT_FIT, { prefix: 'object' }),
    uniqueRule(BG_AND_OBJECT_POSITION, { prefix: 'object' }),
    cardinalRule('overflow', {
        dir: 'x|y',
        overrides: { x: [EMPTY], y: [EMPTY] },
        dash: true,
    }),
    ...
    ...simpleRules('grow|shrink', { def: true }),
    ...cardinalRules('m', {
        dir: 't|r|b|l|x|y|s|e',
        overrides: {
            t: [EMPTY, 'y'],
    ...
    // ------------------
    ...simpleRules('blur|contrast', { def: true }),
    ...simpleRules('mix-blend|stroke|grayscale|cursor', { byType: true }),
    ...simpleRules('opacity|scale|brightness'),
    // ------------------
    arbitraryRule(),
]
